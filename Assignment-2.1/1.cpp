...
struct VAO* create3DObject (GLenum primitive_mode, int numVertices, const GLfloat* vertex_buffer_data, const GLfloat* color_buffer_data, GLenum fill_mode=GL_FILL)
{
    ...
    glBufferData (GL_ARRAY_BUFFER, 2*numVertices*sizeof(GLfloat), color_buffer_data, GL_STATIC_DRAW);  // Copy the vertex colors
    glVertexAttribPointer(
                          1,                  // attribute 1. Color
                          2,                  // size (r,g,b)
                          GL_FLOAT,           // type
...
int fallorcollide()
{
  ...
  if(pos_x<=-12 or pos_x>=12 or pos_z<=-12 or pos_z >=12)
    return 1;
  return 0;
}
...
void draw ()
{
  ...
  glm::mat4 translatecube = glm::translate(glm::vec3(0.0f,15.0f,0.0f));
  ...
  {
    Matrices.model = glm::mat4(1.0f);
    Matrices.model*=(glm::translate(glm::vec3(obs[i][0],4,obs[i][1])));
    MVP = VP*Matrices.model;
    glUniformMatrix4fv(Matrices.MatrixID,1,GL_FALSE,&MVP[0][0]);
    draw3DObject(triangle);
  }
  ...
  {
    Matrices.model = glm::mat4(1.0f);
    Matrices.model*=(glm::translate(glm::vec3(pos_x,4,pos_z)));
    MVP = VP*Matrices.model;
    glUniformMatrix4fv(Matrices.MatrixID,1,GL_FALSE,&MVP[0][0]);
    draw3DObject(triangle);
  }
...
int main (int argc, char** argv)
{
  ...
  if(lives>=0)
  {
	...
	initGL (width, height);
  GLuint Texture = loadBMP_custom("rocky_texture_9_by_zeroempires-d66hd6j.bmp");
    glutMainLoop ();
  }
